namespace KMHPatch.SubProtocol
{
    ...
    {
        // Bump when the wire format changes incompatibly; a client whose version does not match refuses to send.
        public const int CurrentVersion = 2;

        // Reports the true build and gates nothing: compatibility is capability-based, and the wire rides CurrentVersion.
        public const string BuildVersion = "1.3.0";

        // What the player sees (What's New, load log). Diverges from BuildVersion on a client-only patch (e.g. 1.3.0.1).
        ...
        // Shared constants so server and patch agree on the vocabulary; the dispatcher does not enforce them.
        public static class Kind
        {
            ...
            // Payload { level, text }: action feedback that does not warrant a full snapshot.
            public const string Notice      = "kmh.notice";          // server -> client

            // Payload { op }: that op id reached a terminal decision; until it arrives a re-send reuses the id.
            public const string OpResult    = "kmh.op.result";       // server -> client

            // Payload { delivery_id }: this save durably holds that delivery, so the server may stop owing it.
            ...
            // Carry { amount }; the server validates balance and permission, then rebroadcasts the snapshot.
            ...
            // Carry { item_def_name, qty }, validated and rebroadcast on the same terms.
            ...
            // Buy carries { listing_id, qty } and Cancel { listing_id }, both validated against silver and ownership.
            ...
            // The server verifies the caller's caravan really holds the item and qty before escrowing it.
            public const string MarketplacePost     = "kmh.marketplace.post";       // client -> server

            // The server filters guild-only quests by the caller's membership before sending.
            ...
            // Each carries { quest_id }; the server answers with a fresh snapshot rather than a per-mutation reply.
            ...
            // The server escrows the bounty silver and assigns the id, so the composer's fields are a proposal only.
            public const string QuestPost           = "kmh.quest.post";             // client -> server

            // The server resolves which guild from the caller's own membership.
            ...
            // Each carries { username }; the server validates the caller's rank against the target's before applying.
            ...
            // Carries { perk_key }; the server deducts from the guild treasury and bumps the level.
            public const string GuildBuyPerk        = "kmh.guild.buy_perk";         // client -> server
            // Carries { motd } (empty string clears). Admin-only.
            ...
            // Each carries { other_guild }: None -> AlliedRequested -> Allied -> None, and any -> Hostile -> None.
            ...
            // Carries the full GuildSettingsDto; the server range-checks tax and caps before rebroadcasting.
            ...
            // The server pushes this after handshake and on every link change, so the client never polls.
            ...
            // Every guild on the server, unlike GuildSnapshot, which is scoped to the caller's own.
            ...
            // { quest_id, total } where total is cumulative, so the server keeps the max per user rather than summing.
            public const string WorldContribute     = "kmh.world.contribute";       // client -> server
            // The goods leave the caravan first, and anything the server cannot credit is not given back.
            ...
            // Attachments escrow from the sender's treasury until the recipient claims or declines them.
            ...
            // KMH chat - live channels (server-wide first, guild/DM later). Rides the KMH sub-protocol, not RWT chat.
            ...
            // The message carries only an id, so converted bytes never travel in a chat line or its history.
            ...
            // Discord signs CDN links for 24h and chat history keeps a message for 48, so a persisted link dies first.
            ...
            // Frontier Operations. The server asks where an outpost could go; the client answers deterministically.
            ...
            // Roadworks - the road network a Roadworks site builds.
            ...
            // The server accumulates the union, so friendly names work for items its own headless build has no def for.
            public const string ItemLabels             = "kmh.item_labels";              // client -> server
            public const string SiteMetaPush           = "kmh.site_meta";                // client -> server
            // A separate envelope so it never bloats the already near-cap labels push.
            ...
            // Sent only when the player enables it, or consents after the server asks via debug_uplink in the hello.
            public const string DebugLog               = "kmh.debug.log";                // client -> server

            // The full profile for hard enforcement rides the chunked profile.* kinds rather than this snapshot.
